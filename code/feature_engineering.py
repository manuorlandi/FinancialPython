...
def z_score_scale(col: str, suffix: str) -> pl.Expr:
    return (
        ...
        pl.col(col).std()).alias(f"{col}{suffix}")
...
def calculate_bollinger_bands(
    ...
    df = df.with_columns([
        (pl.col("sma") + (k * pl.col("std_dev"))).alias("upper_band").round(2),
        (pl.col("sma") - (k * pl.col("std_dev"))).alias("lower_band").round(2)
    ])
...
def calculate_pacf(df, col, lags, alpha=.05, keep_zero_lag=False,plot=False):   
    ...
    if plot:
        ...
        plot_pacf(df.select(col), lags=lags, zero=False,ax=ax)  # Specify number of lags to visualize
        plt.show()

    return significant_lags


def calculate_acf(df, col, lags, alpha=.05, keep_zero_lag=False, plot=False):
    z_score = norm.ppf((1 - alpha/2)).round(2)
...
def get_all_data(start_date, end_date):
    ...
    # Merge all data
    # all_data = stock_data.join(macro_data, on='DATE', how='outer')
    # return all_data.sort('DATE')
...
if __name__ == "__main__":
    ...
    df = df.with_columns(
        pl.when(pl.col("open") > pl.col("close")).then(1).otherwise(0)
        .alias("green_candle"),
        pl.when(pl.col("open") <= pl.col("close")).then(1).otherwise(0)
        .alias("red_candle")
    )
